...
describe "queries by single attribute for a model of a specified type when model does not have an ancestor" do
  ...
  it "should find first model that matches specified model attribute and return model class for queries from model class" do
    mod = ParentModel.find_by_model(:first, :conditions => "parent_models.parent_model_string = '#{model_data[:parent_model_1]['parent_model_string']}'")
    mod.should have_attributes_with_values(model_data[:parent_model_1])
    mod.should be_class(ParentModel)
  end
...
describe "queries by single attribute for a model of a specified type when model has an ancestor" do
  ...
  it "should find first ancestor model that matches specified ancestor model attribute and return ancestor model class for queries from ancestor model class" do
    mod = ParentModel.find_by_model(:first, :conditions => "parent_models.parent_model_string = '#{model_data[:child_model_1]['parent_model_string']}'")
    mod.should have_attributes_with_values(model_data[:parent_child_model_1])
    mod.should be_class(ParentModel)
  end
  ...
  it "should find first model that matches specified ancestor model attribute and return model class for queries from model class" do
    mod = ChildModel.find_by_model(:first, :conditions => "parent_models.parent_model_string = '#{model_data[:child_model_1]['parent_model_string']}'")
    mod.should have_attributes_with_values(model_data[:child_model_1])
    mod.should be_class(ChildModel)
  end

  it "should find first model that matches specified model attribute and return model class for queries from model class" do
    mod = ChildModel.find_by_model(:first, :conditions => "child_models.child_model_string = '#{model_data[:child_model_1]['child_model_string']}'")
    mod.should have_attributes_with_values(model_data[:child_model_1])
    mod.should be_class(ChildModel)
  end
...
describe "queries by single attribute for a model of a specified type when model has an ancestor with an ancestor" do
  ...
  it "should find first ancestor's ancestor model that matches specified ancestor's ancestor model attribute and return ancestor's ancestor model class for queries from ancestor's ancestor model class" do
    mod = ParentModel.find_by_model(:first, :conditions => "parent_models.parent_model_string = '#{model_data[:grandchild_model_1]['parent_model_string']}'")
    mod.should have_attributes_with_values(model_data[:parent_grandchild_model_1])
    mod.should be_class(ParentModel)
  end
  ...
  it "should find first ancestor model that matches specified ancestor's ancestor model attribute and return ancestor model class for queries from ancestor model class" do
    mod = ChildModel.find_by_model(:first, :conditions => "parent_models.parent_model_string = '#{model_data[:grandchild_model_1]['parent_model_string']}'")
    mod.should have_attributes_with_values(model_data[:child_grandchild_model_1])
    mod.should be_class(ChildModel)
  end

  it "should find first ancestor model that matches specified ancestor model attribute and return ancestor model class for queries from ancestor model class" do
    mod = ChildModel.find_by_model(:first, :conditions => "child_models.child_model_string = '#{model_data[:grandchild_model_1]['child_model_string']}'")
    mod.should have_attributes_with_values(model_data[:child_grandchild_model_1])
    mod.should be_class(ChildModel)
  end
  ...
  it "should find first model that matches specified ancestor's ancestor model attribute and return model class for queries from model class" do
    mod = GrandchildModel.find_by_model(:first, :conditions => "parent_models.parent_model_string = '#{model_data[:grandchild_model_1]['parent_model_string']}'")
    mod.should have_attributes_with_values(model_data[:grandchild_model_1])
    mod.should be_class(GrandchildModel)
  end

  it "should find first model that matches specified ancestor model attribute and return model class for queries from model class" do
    mod = GrandchildModel.find_by_model(:first, :conditions => "child_models.child_model_string = '#{model_data[:grandchild_model_1]['child_model_string']}'")
    mod.should have_attributes_with_values(model_data[:grandchild_model_1])
    mod.should be_class(GrandchildModel)
  end

  it "should find first model that matches specified model attribute and return model class for queries from model class" do
    mod = GrandchildModel.find_by_model(:first, :conditions => "grandchild_models.grandchild_model_string = '#{model_data[:grandchild_model_1]['grandchild_model_string']}'")
    mod.should have_attributes_with_values(model_data[:grandchild_model_1])
    mod.should be_class(GrandchildModel)
  end
